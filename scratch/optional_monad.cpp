...
/* implicit */ operator co_await(std::optional<Value> o) {
...
inline std::optional<int> f1() {
    return 5;
}

inline std::optional<double> f2(int x) {
    return 2.0 * x;
}

inline std::optional<int> f3(int x, double y) {
    return x * y;
}

inline std::optional<int> f() {
    auto x = co_await co_awaitUnwrap(f1());
    auto y = co_await co_awaitUnwrap(f2(x));
    auto z = co_await co_awaitUnwrap(f3(x, y));
    co_return z;
}
